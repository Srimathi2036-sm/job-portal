...
class Job extends Component {
  ...
  componentDidMount = () => {
    ...
    axios
      ...
      .then((res) => {
        console.log("Full Job Data Received:", res.data);
  
        if (!res.data || !res.data.applications) {
          console.error("No applications found in response!");
        } else {
          console.log("Applications Received:", res.data.applications);
          console.log("Applications in state:", this.state.display.applications);
      ...
      .catch((err) =>
        Swal.fire({
          ...
          footer: JSON.stringify(err.response?.data || "Unknown error"),
        })
      );
  };
  ...
  filterApplicationSort = (event) => {
    const sortBy = event.target.value;
    let sortedApplications = [...this.state.display.applications];
    ...
    if (sortBy === "name") {
      ...
      sortedApplications.sort((a, b) => 
        (b.applicantDets.rating / (b.applicantDets.ratedBy.length || 1)) -
        (a.applicantDets.rating / (a.applicantDets.ratedBy.length || 1))
      );
    }
                      ...
                      <div className="job-detailed-content p-5">
                        ...
                        <div className="mt-5 table-responsive">
                          <table className="table table-hover">
                            ...
                            <tbody>
                              <tr>
                                <td className="text-center">{this.state.display.job ? this.state.display.job.duration : ""} year</td>
                                ...
                                <td className="text-center">₹ {this.state.display.job ? this.state.display.job.salary : ""} / year</td>
                              </tr>
                            </tbody>
                          </table>
                        </div>
 ...
 {/* Added table-sm */}
    ...
    <tbody>
      {this.state.display.applications && this.state.display.applications.map((application, index) => (
        <tr key={index} style={{ fontSize: "14px" }}> {/* Reduced font size */}
          ...
          <td className="text-center">
            <button className="btn btn-sm btn-info" onClick={() => this.viewApplication(application.applicant, application.application, application.applicantDets)}>VIEW</button>
          </td>
        </tr>
      ))}
    </tbody>
